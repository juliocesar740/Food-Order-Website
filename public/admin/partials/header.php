<div class="header">
   <div class="nav">
      <ul>
         <li><a href="/admin/index_admin.php" class="admin-nav-links">Dashboard</a></li>
         <li><a href="/admin/manage_admin.php" class="admin-nav-links">Admin</a></li>
         <li><a href="/admin/manage_category.php" class="admin-nav-links">Category</a></li>
         <li><a href="/admin/manage_food.php" class="admin-nav-links">Food</a></li>
         <li><a href="/admin/manage_order.php" class="admin-nav-links">Order</a></li>
         <li><a href="/admin/logout.php" class="admin-nav-links">Logout</a></li>
      </ul>
   </div>
</div>